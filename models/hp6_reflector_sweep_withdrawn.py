"""
Compact fast-spectrum microreactor model (reflector sweep, withdrawn case)
...
- Append k-effective and reflector worth summary data to a CSV.
...
- results/reflector_sweep_withdrawn.csv (appends one row per case)
"""
...
def main() -> int:
    ...
    csv_path = "results/reflector_sweep_withdrawn.csv"
    ...
    for idx, reflector_thickness in enumerate(refl_values):
        CASE_NAME = f"hp6_rod_withdrawn_Na_T{int(round(Tfuel))}_refl{reflector_thickness:.1f}".replace(".", "p")
